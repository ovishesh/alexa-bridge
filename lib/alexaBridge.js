...
appInsights.setup("e578976e-a267-48b7-8036-162b4c00240d").start(); // reads in from APPINSIGHTS_INSTRUMENTATIONKEY by default in the .env file
...
function botSays(activity) {
  console.log("Subscribe activity: " + activity);
...
function alexaSays(req, res, bot, next) {
  ...
  if (req.body && req.body.request && req.body.request.type &&
    ...
    return next(new errors.UnauthorizedError({body: {foo: 'bar', name: 'john doe', message: 'whatever error message'}}));
  }
}
